...
void speedControl() {
	...
	speed = fabs(127-(rawTorque));						// Absolute value of how far away from the midline you are.
...
void horizontalBar() {
	if (oscillationMode == 1) {
		oscillationFn();
		glTranslatef(oscillation,window_height/4,0);		// shifts along x-axis
	} else {
		glTranslatef(shift,window_height/4,0);				// shifts along x-axis
	}
...
void solidCircle() {
	if (oscillationMode == 1) {
		oscillationFn();
		glTranslatef(oscillation, window_height/1.7, 0.0);					// Shift along the x axis.
	} else {
		glTranslatef(shift, window_height/1.7, 0.0);					// Shift along the x axis.
	}
...
int serialport_read(int fd, unsigned char* serialbuffer) {
	...
	if (n==-1) {
		...
		fprintf(stderr, "error:  no data available (flooding the serial port)  Stopping.\n");
		displayMode = 0;
		runMode = 0;
		glutPostRedisplay();
		return 0;
